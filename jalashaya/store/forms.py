...
class OrderCreateForm(forms.ModelForm):
    ...
    selected_address = forms.CharField(required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        branch_field = self.fields.get("branch")
        if branch_field is not None:
            branch_field.queryset = Branch.objects.filter(is_active=True).select_related("state")
        ...
        delivery_address_field = self.fields.get("delivery_address")
        if delivery_address_field is not None:
            delivery_address_field.widget = forms.HiddenInput()
            delivery_address_field.required = False

        selected_address_field = self.fields.get("selected_address")
        if selected_address_field is not None:
            selected_address_field.widget = forms.Select(
                attrs={"class": "form-control", "id": "id_selected_address"},
                choices=[("", "Add New Address")],
            )
            selected_address_value = (self.data.get("selected_address") or "").strip() if self.is_bound else ""
            if selected_address_value:
                selected_address_field.widget.choices = [
                    ("", "Add New Address"),
                    (selected_address_value, "Selected saved address"),
                ]

        save_address_field = self.fields.get("save_address")
        if save_address_field is not None:
            save_address_field.widget.attrs.update({"id": "id_save_address"})
